...
rule all:
    input:
        expand(output_path + "{sample}_fusions.out", sample=sample),
        expand(output_path + "{sample}_fusions_pass.out", sample=sample),
        expand(output_path + "{sample}_peptides.out",sample=sample)
...
rule sort_bam:
    ...
    output:
        fusions = os.path.join(output_path, "{sample}_fusions.out"),
        fusions_discarded = os.path.join(output_path, "{sample}_fusions_discarded.out")
    params:
...
rule loose_filter:
    input:
        fusions = os.path.join(output_path, "{sample}_fusions.out")
    output:
        fusions_filtered = os.path.join(output_path, "{sample}_fusions_pass.out")
    shell:
        """
...
rule generate_peptides:
    input:
        fusions = os.path.join(output_path, "{sample}_fusions_pass.out")
    output:
